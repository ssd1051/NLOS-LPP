...
This is view_as_real version to allow NCCL backend
"""
...
from complexFunctionsAsReal import (
    complex_relu,
    complex_leaky_relu,
    complex_tanh,
    complex_sigmoid,
...
class ComplexInstanceNorm2d(Module):
    ...
    def forward(self, input):
        return torch.complex(self.instance_norm_real(input.real), self.instance_norm_imag(input.imag))
...
class NaiveComplexLayerNorm(Module):
    ...
    def forward(self, input):
        return torch.complex(self.layer_norm_real(input.real), self.layer_norm_imag(input.imag))
...
class ComplexUpSamplingBlock(Module):
    def __init__(self, 
        ...
        self.up = ComplexConvTranspose3d(in_channels, in_channels // 2, kernel_size=2, stride=2, output_padding=first_conv_padding)
        self.conv = ComplexUnetConvBlock(in_channels // 2, out_channels)
    ...
    def forward(self, inputs1, inputs2):
        ...
        outputs = self.conv(inputs1 + inputs2)
        return outputs
...
def normalize_min_max(input_real, input_imag):
    # min-max scaling
    bd, t2, h2, w2 = input_real.shape
    data_bxtxk_real = input_real.reshape(bd, t2, -1)
    data_bxtxk_imag = input_imag.reshape(bd, t2, -1)

    # most are 0
    data_min = data_bxtxk_real.min(2, keepdim=True)[0]
    data_max = torch.abs(data_bxtxk_real).max(2, keepdim=True)[0]
    data_norm_real = (data_bxtxk_real - data_min) / (data_max + 1e-15)
    data_norm_imag = (data_bxtxk_imag - data_min) / (data_max + 1e-15)

    return data_norm_real.view(bd, t2, h2, w2), data_norm_imag.view(bd, t2, h2, w2), data_min, data_max


def un_normalize_min_max(input, data_min, data_max):
    bd, t2, h2, w2 = input.shape
    data_bxtxk = input.reshape(bd, t2, -1)

    data_origin = (data_bxtxk * (data_max + 1e-15)) + data_min

    return data_origin.view(bd, t2, h2, w2)


def normalize_max(input_real, input_imag):
    # max scaling
    bd, t2, h2, w2 = input_real.shape
    data_bxtxk_real = input_real.reshape(bd, t2, -1)
    data_bxtxk_imag = input_imag.reshape(bd, t2, -1)
    ...
    # most are 0
    data_max = torch.abs(data_bxtxk_real).max(2, keepdim=True)[0]
    data_norm_real = data_bxtxk_real / (data_max + 1e-15)
    data_norm_imag = data_bxtxk_imag / (data_max + 1e-15)

    return data_norm_real.view(bd, t2, h2, w2), data_norm_imag.view(bd, t2, h2, w2), data_max, data_max


def un_normalize_max(input, useless, data_max):
    bd, t2, h2, w2 = input.shape
    data_bxtxk = input.reshape(bd, t2, -1)

    data_origin = data_bxtxk * (data_max + 1e-15)

    return data_origin.view(bd, t2, h2, w2)


def normalize_z_score(input_real, input_imag):
    # Reshape input for Z-score normalization
    b, d, t2, h2, w2 = input_real.shape
    data_bxtxk_real = input_real.reshape(b, d, -1)
    data_bxtxk_imag = input_imag.reshape(b, d, -1)

    # Calculate mean and std
    mean = data_bxtxk_real.mean(2, keepdim=True)
    std = data_bxtxk_real.std(2, keepdim=True)

    # Z-score normalization
    data_norm_real = (data_bxtxk_real - mean) / (std + 1e-15)
    data_norm_imag = (data_bxtxk_imag - mean) / (std + 1e-15)

    return data_norm_real.view(b, d, t2, h2, w2), data_norm_imag.view(b, d, t2, h2, w2), mean, std


def un_normalize_z_score(input, mean, std):
    b, d, t2, h2, w2 = input.shape
    data_bxtxk = input.reshape(b, d, -1)

    data_origin = (data_bxtxk * (std + 1e-15)) + mean

    return data_origin.view(b, d, t2, h2, w2)


def normalize_robust(input_real, input_imag):
    # Reshape input for robust scaling
    bd, t2, h2, w2 = input_real.shape
    data_bxtxk_real = input_real.reshape(bd, t2, -1)
    data_bxtxk_imag = input_imag.reshape(bd, t2, -1)

    # Calculate the first and third quartiles (25th and 75th percentiles)
    q1 = torch.quantile(data_bxtxk_real, 0.25, dim=2, keepdim=True)
    q3 = torch.quantile(data_bxtxk_real, 0.75, dim=2, keepdim=True)

    # Compute the interquartile range (IQR)
    iqr = q3 - q1

    # Robust scaling
    data_norm_real = (data_bxtxk_real - q1) / (iqr + 1e-15)
    data_norm_imag = (data_bxtxk_imag - q1) / (iqr + 1e-15)

    return data_norm_real.view(bd, t2, h2, w2), data_norm_imag.view(bd, t2, h2, w2), q1, iqr


def un_normalize_robust(input, q1, iqr):
    bd, t2, h2, w2 = input.shape
    data_bxtxk = input.reshape(bd, t2, -1)

    data_origin = (data_bxtxk * (iqr + 1e-15)) + q1

    return data_origin.view(bd, t2, h2, w2)


def normalize_complex(input, method):
    real_part = input.real
    imag_part = input.imag

    normalized_real, normalized_imag, p1, p2 = method(real_part, imag_part)

    normalized_data = torch.complex(normalized_real, normalized_imag)

    return normalized_data, p1, p2

def normalize_complex_as_real(input, method):
    real_part = input.real
    imag_part = input.imag
...
def un_normalize_complex(input, method, p1, p2):
    real_part = input.real
    imag_part = input.imag

    un_normalized_real = method(real_part, p1, p2)
    un_normalized_imag = method(imag_part, p1, p2)

    un_normalized_data = torch.complex(un_normalized_real, un_normalized_imag)

    return un_normalized_data


def un_normalize_complex_as_real(input, method, p1, p2):
    real_part = input[..., 0]
    imag_part = input[..., 1]

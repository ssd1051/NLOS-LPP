...
from utils_network_adp import AdaptiveVirtualWave, UNet, DCNet
...
class phasor(nn.Module):
    ...
    def __init__(self, spatial=256, crop=512, \
        ...
        self.adp_virtual_wave = AdaptiveVirtualWave(self.bin_resolution, self.virtual_wavelength, self.cycles)
        self.unet = UNet()
        self.dcnet = DCNet()
    ...
    def parpareparam(self):
        ...
        # 注意：虽然波是垂直传播的，但是实际上波前在水平方向上的投影仍然是波本身的波长，所以需要小于二分之一的中继墙采样间隔
        # cycles = 5;  # number of wave cycles in the wavelet, typically 4-6
        
        s_lamda_limit = wall_size / (sptial_grid - 1);  # sample spacing on the wall
        virtual_wavelength = sampling_coeff * (s_lamda_limit * 2);  # virtual wave length in units of cm
    ...
    def forward(self, feture_bxdxtxhxw, tbes, tens):
        ...
        data_BDxTxHxW, alpha = self.dcnet(data_BDxTxHxW.view(bnum, dnum, tnum, hnum, wnum), 
                                          gridz_1xMx1x1_1.reshape(1, 1, -1, 1, 1),
                                          gridz_1xMx1x1_2.reshape(1, 1, -1, 1, 1),
                                          gridz_1xMx1x1_4.reshape(1, 1, -1, 1, 1),) # distance compensation
        ...
        # datafre = torch.view_as_real(torch.fft.fftn(datapad_2BDx2Tx2Hx2W, dim=(-3, -2, -1), norm="ortho"))
        datafre = torch.fft.fftn(datapad_2BDx2Tx2Hx2W, dim=(-3, -2, -1), norm="ortho")
        
        datafre = self.unet(datafre.view(2 * bnum, dnum, temprol_grid * 2, sptial_grid * 2, sptial_grid * 2))  # x_t, x_f
        datafre = torch.view_as_real(datafre.view(2 * bnum * dnum, self.crop * 2, hnum * 2, wnum * 2))
        ...
        return volumn_BxDxTxHxW, 0,0,alpha

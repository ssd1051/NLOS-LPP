...
def definePsf_single(sptial_grid, temprol_grid, slope):
    ...
    # -0.5 to 0.5
    x_2N = np.arange(sptial_grid, dtype=np.float32)
    x_2N = x_2N / (sptial_grid - 1) * 2 - 1
    x_2N = x_2N * 0.5
    ...
    # 0 to 1
    z_2M = np.arange(temprol_grid, dtype=np.float32)
    z_2M = z_2M / (temprol_grid - 1)
    ...
    # fft shift, key point is N/2
    f1_2Nx2NX2M = np.roll(e_2Nx2NX2M, shift=int(N/2), axis=0)
    f2_2Nx2NX2M = np.roll(f1_2Nx2NX2M, shift=int(N/2), axis=1)
    if debug:
        cv2.imshow("0", f2_2Nx2NX2M[:, :, 0] * 256)
        cv2.imshow("1", f2_2Nx2NX2M[:, :, M // 2 - 1] * 256)
        cv2.imshow("2", f2_2Nx2NX2M[:, :, M - 1] * 256)
